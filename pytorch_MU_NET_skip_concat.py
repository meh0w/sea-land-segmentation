...
class decoder(nn.Module):
    def __init__(self, encoder_channels=[3,64,128,256,512],bilinear=True,base_c: int = 64):
        ...
        self.d1 = BasicLayer(dim=sum(encoder_channels[:5])+encoder_channels[4]// factor, depth=2, num_heads=2)
        self.up1 = UpsampleConv(2*sum(encoder_channels[:5])+encoder_channels[4], sum(encoder_channels[:5])+encoder_channels[4] // factor, bilinear)
        self.up2 = Up(sum(encoder_channels[:5])+encoder_channels[4]+sum(encoder_channels[:3]), (sum(encoder_channels[:5])+encoder_channels[4]+sum(encoder_channels[:3])) // factor, bilinear)
        self.up3 = Up((sum(encoder_channels[:5])+encoder_channels[4]+sum(encoder_channels[:3])) // factor + sum(encoder_channels[:3]), ((sum(encoder_channels[:5])+encoder_channels[4]+sum(encoder_channels[:3])) // factor + sum(encoder_channels[:3])) // factor, bilinear)
        self.up4 = Up((((sum(encoder_channels[:5])+encoder_channels[4]+sum(encoder_channels[:3])) // factor + sum(encoder_channels[:3])) // factor) + sum(encoder_channels[:2]), base_c, bilinear)

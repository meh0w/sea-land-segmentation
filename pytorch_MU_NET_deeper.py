...
class AMM(nn.Module):
    def __init__(self, dim=64):
        super().__init__()
        self.spatial_branch = nn.Sequential(nn.Conv2d(dim, dim, kernel_size=3, padding=1, groups=dim),
...
class encoder(nn.Module):
    def __init__(self, n_channels=3, encoder_channels=[3,32,64,128,256,512]):
        ...
        self.down2_1 = Down(encoder_channels[3], encoder_channels[4])
        self.down3 = DownsampleConv(encoder_channels[4], encoder_channels[5])
        self.down4 = DownsampleConv(encoder_channels[5], encoder_channels[5])

        self.e4 = BasicLayer(dim=encoder_channels[5], depth=1, num_heads=2)
        self.e5 = BasicLayer(dim=encoder_channels[5], depth=1, num_heads=2)
    def forward(self, x):
        outs = []
        x1 = self.inc(x)
        outs.append(x1)
        ...
        x3_1 = self.down2_1(x3)  # 1/4
        outs.append(x3_1)

        x4 = self.down3(x3_1)  # 1/8
        x4=self.e4(x4)
        outs.append(x4)
...
class decoder(nn.Module):
    def __init__(self, encoder_channels=[3,32,64,128,256,512],bilinear=True,base_c: int = 64):
        super(decoder, self).__init__()
        
        factor = 2 if bilinear else 1
        self.d1 = BasicLayer(dim=encoder_channels[5], depth=1, num_heads=2)
        self.up1 = UpsampleConv(encoder_channels[5]*2, encoder_channels[5], bilinear)
        self.up2 = Up(encoder_channels[5] + encoder_channels[4] , encoder_channels[4], bilinear)
        self.up2_1 = Up(encoder_channels[4] + encoder_channels[3], encoder_channels[3], bilinear)
        ...
        self.attn2_1 = AMM(dim=encoder_channels[4])  # C3  256
        self.attn3 = AMM(dim=encoder_channels[5])  # C4  512
    def forward(self,x,h,w):
        c1,c2,c3,c3_1,c4,c5=x[:6]
        B, _, H, W = c5.shape
        c2 = self.attn1(c2)
        c3 = self.attn2(c3)
        c3_1 = self.attn2_1(c3_1)
        c4 = self.attn3(c4)
        x = self.up1(c5, c4)
        x = self.d1(x)
        x = self.up2(x, c3_1)

...
from pytorch_metrics import all_metrics, All_metrics
...
def run(c):
    try:
        ...
        if DATASET == 'SWED':
            ...
            data_set_train = DataLoaderSWED(img_files[idx], label_files[idx], False)
            data_set_valid = DataLoaderSWED(np.delete(img_files, idx), np.delete(label_files, idx), False)
...
if __name__ == '__main__':
    ...
    configs = [
        ...
        "MOMENTUM": 0.9,
        "WEIGHT_DECAY": 1e-4,
        "NOTE": 'encoder_channels = [32,64,128,256]'
        },
    ]
    for cc in configs:

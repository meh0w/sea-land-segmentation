...
def get_model(input_size, batch_size, format='channels_last'):
    ...
    # Branching out
    output_seg = Conv2D(2, (1,1), padding='valid', data_format=format)(relu)
    smax_seg = Softmax(axis=-1, name='output_seg')(output_seg)

    output_edge = Conv2D(2, (1,1), padding='valid', data_format=format)(relu)
    smax_edge = Softmax(axis=-1, name='output_edge')(output_edge)
    ...
    model = Model(inputs=nn_in, outputs=[smax_seg, smax_edge], name="DeepUNet2")
